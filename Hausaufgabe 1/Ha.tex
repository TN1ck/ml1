...
\end{enumerate}
\subsubsection*{b)}
To satisfy the constrain of $P(w_2 \mid x) = P(error \mid x)$, $P(w_1 \mid x)$ must contain $P(w_2 \mid x)$. Visually it would mean that the area of the curve of $P(w_2 \mid x)$ is completly included inside the area of the curve of $P(w_1 \mid x)$. When varying the mean $\mu$, both curves move away from each other, which means it always has to stay 0, else it's not possible to satisfy the constrain. The scale $\sigma$ can take any value that is larger than zero, it has the same effect on both distributions. $P(w_1)$ must always be larger than $P(w_2)$ (or else $P(w_1 \mid x)$ can't contain $P(w_2 \mid x)$).

Formally all possible value-tuples are:

$$\{ (P(w_1), P(w_2), \mu, \sigma) \mid P(w_1) \ge P(w_2) \land \mu = 0 \land \sigma \in \R^+ \}$$
